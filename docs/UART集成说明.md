...
### 3.1 hello：周期打印 `Hello, EES-338!`（约 0.3s 一次）
- 上电立即打印，之后约 0.3s 重复一次（纯延时计数实现）；
- 程序要点：`auipc` 得到 UART 基址与数据区基址 → 逐字取字符串
  （`uart_hello_mem.hex`，内容为一条 Hello）→ 轮询 TX 空闲后 `sw` 发送 → 发完延时再发。

### 3.2 echo：PC 发字符，CPU 原样回显
轮询 `UART_STATUS.bit1`（RX 可读）→ `lw UART_DATA` 读字符
→ 等 TX 空闲 → `sw UART_DATA` 回发。用串口助手发送即可看到回显。

### 3.3 helloecho：**周期打印 Hello（约 2s 一次）+ 随时 RX 回显**（日常/自检推荐）
- 上电立即打印一条 Hello，之后约 **2s** 重复一次；
- 同时**每一轮主循环都轮询 RX**：PC 任何时候发来字节都会立即原样回显
  （回显优先，与打印互不干扰）；
- 一次下载即可同时验证 TX 与 RX+TX（回显）。
...
已归档（不会被重新综合清掉）：
```
bitstream/EES338Top_hello.bit        hello：约0.3s周期打印一条 Hello（TX 测试）
bitstream/EES338Top_helloecho.bit    hello+echo：约2s打印一条 + 随时回显（TX/RX 全测，推荐）
```
...
2. 打开串口助手，应**约每 2s 收到一行** `Hello, EES-338!`；
