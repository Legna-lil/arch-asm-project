...
module tb_EES338_helloecho;
    ...
    EES338Top #(
        .HEX_FILE ("E:/VivadoProject/Lab2/Lab2.file/uart_helloecho.hex"),
    ...
    initial begin
        ...
        // 1) 等待两条 Hello 打印完成（34B, ~3ms）
        for (watch = 0; watch < 5000; watch = watch + 1) begin
            if (got_cnt >= 34) watch = 5000;
            else #1000;
        end
        ...
        // 2) 回显测试（回显字节从索引 34 开始）：逐字节发送并等回显
        for (idx = 0; idx < 4; idx = idx + 1) begin
            send_byte(sent[idx]);
            for (watch = 0; watch < 3000; watch = watch + 1) begin
                if (got_cnt >= 35 + idx) watch = 3000;
                else #1000;
            end
            if (got_cnt >= 35 + idx) begin
                if (got_chars[34+idx] == sent[idx])
                    $display("PASS: echo[%0d] = 0x%02X (%c)", idx,
                             got_chars[34+idx], got_chars[34+idx]);
                else begin
                    $display("FAIL: echo[%0d] expected 0x%02X got 0x%02X",
                             idx, sent[idx], got_chars[34+idx]);
                    err_cnt = err_cnt + 1;
                end
            end
            else begin
